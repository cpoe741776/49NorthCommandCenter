// src/components/DisregardedArchiveModal.jsx
...
const DisregardedArchiveModal = ({ isOpen, onClose, emails, onRevive, onRefresh }) => {
  ...
  const handleRevive = async (email, newRecommendation) => {
    if (!window.confirm(`Revive this email as "${newRecommendation}"?\n\nIt will move back to Active_Bids.`)) return;
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-6xl w-full max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">Review and revive previously disregarded emails</p>
          </div>
              ...
              <div className="divide-y divide-gray-200">
                {emails.map((email) => (
                  <div
                    ...
                    className={`p-4 cursor-pointer transition-colors ${selectedEmail?.id === email.id ? 'bg-orange-50 border-l-4 border-orange-600' : 'hover:bg-gray-50'}`}
                  >
                    <div className="flex items-start justify-between mb-2">
                      ...
                      <AlertCircle size={16} className="text-orange-500 ml-2" />
                    </div>
                    <div className="flex items-center gap-2 text-xs text-gray-600">
                      <Calendar size={12} />
                      {fmt(email.emailDateReceived)}
                    </div>
                    {email.scoreDetails && <div className="mt-2 text-xs text-gray-500">Score: {email.scoreDetails}</div>}
                  </div>
                ))}
              </div>
            )}
                  ...
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex-1">
                      ...
                      <div className="space-y-1 text-sm">
                        <div className="flex items-center gap-2 text-gray-600"><span className="font-semibold">From:</span><span>{selectedEmail.emailFrom}</span></div>
                        {selectedEmail.bidSystem !== 'Unknown' && (
                          <div className="flex items-center gap-2 text-gray-600"><span className="font-semibold">System:</span>
                            <span className="px-2 py-1 bg-gray-100 text-gray-700 rounded text-xs font-medium">{selectedEmail.bidSystem}</span>
                          </div>
                        )}
                        <div className="flex items-center gap-2 text-gray-600"><span className="font-semibold">Date:</span><span>{fmt(selectedEmail.emailDateReceived)}</span></div>
                        {selectedEmail.scoreDetails && <div className="flex items-center gap-2 text-gray-600"><span className="font-semibold">Score:</span><span>{selectedEmail.scoreDetails}</span></div>}
                      </div>
                    </div>
                    ...
                    <div className="mb-4">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {selectedEmail.keywordsFound.split(',').map((kw, i) => (
                          <span key={i} className="px-2 py-1 bg-blue-100 text-blue-700 rounded text-xs">{kw.trim()}</span>
                        ))}
                      </div>
                    </div>
                  )}
                    ...
                    <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
                      {selectedEmail.url !== 'Not provided' && (
                        <div>
                          ...
                          <a href={selectedEmail.url} target="_blank" rel="noopener noreferrer" className="text-sm text-blue-600 hover:underline truncate block">
                            {selectedEmail.url}
                          </a>
                        </div>
                      )}
